...
#   ./brewfile.sh cleanup   [--file PATH] [--force]
#
# Commands:
...
#   cleanup  Preview packages not in the Brewfile; --force removes them
#
# Exit codes:
...
while (( $# > 0 )); do
  case "$1" in
    dump|check|install|diff|cleanup)
...
# --- preflight -------------------------------------------------------------
# A dry run writes nothing, so like --help it should answer on a machine that
# could not do the real work; only a real run needs a Mac with Homebrew on it.
preflight_fail() {
  if (( DRY_RUN == 1 )); then
    warn "$1"
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  preflight_fail "this script targets macOS (detected: $(uname -s))"
fi
if ! command -v brew >/dev/null 2>&1; then
  preflight_fail "Homebrew is not installed or not on PATH"
fi
...
if ! brew bundle --help >/dev/null 2>&1; then
  preflight_fail "'brew bundle' is unavailable — update Homebrew (brew update)"
fi
...
case "$CMD" in
  dump)
    ...
    # dump accepted --dry-run and wrote the file anyway — the same defect
    # packages.sh had on its own dump path: the flag was parsed and then never
    # consulted here, so the mode that promises to touch nothing replaced
    # whatever was at --file.
    if (( DRY_RUN == 1 )); then
      printf "  %s(dry-run)%s would run: brew bundle dump --file=%s --force\n" \
        "$C_DIM" "$C_RESET" "$FILE"
  ...
  cleanup)
    ...
    if (( DRY_RUN || FORCE == 0 )); then
      info "preview only: packages outside $FILE (nothing will be removed)"
      # Homebrew exits 1 when a preview found removals. Accept that one
      # documented verdict only when the expected rerun instruction is present;
      # transport, Brewfile, permission, and bundle failures must stay visible.
      set +e
      cleanup_output="$(brew bundle cleanup --file="$FILE" </dev/null 2>&1)"
      cleanup_rc=$?
      set -e
      [[ -n "$cleanup_output" ]] && printf '%s\n' "$cleanup_output"
      if (( cleanup_rc != 0 )) \
         && ! { (( cleanup_rc == 1 )) \
                && grep -Fq 'Run `brew bundle cleanup --force` to make these changes.' <<<"$cleanup_output"; }; then
        err "brew bundle cleanup preview failed"
        exit 1
      fi
      if (( DRY_RUN && FORCE )); then
        info "--dry-run overrides --force; nothing was removed"
      else
        info "review the list, then rerun with --force to remove it"
      fi
      exit 0
    fi
    warn "removing packages not declared in $FILE"
    if brew bundle cleanup --file="$FILE" --force; then
      ok "removed packages outside the Brewfile"
    else
      err "brew bundle cleanup reported errors"
      exit 1
    fi
    ;;
esac
