...
#                         [--skip-go] [--skip-helm]
#                         [--only python,terraform,go,helm] [--list-tools]
...
${C_BOLD}General options:${C_RESET}
  ...
  --only TOOLS              Install only a comma-separated subset of:
                            python, terraform, go, helm
  --list-tools              Print selectable tool ids and exit
...
while (( $# > 0 )); do
  case "$1" in
    ...
    --only=*)
      ONLY_TOOLS="${1#*=}"
      [[ -n "$ONLY_TOOLS" ]] || { err "--only needs a value"; exit 3; }
      ;;
    ...
    --skip-python)            SKIP_PYTHON=1; EXPLICIT_SKIP=1 ;;
    --skip-terraform)         SKIP_TERRAFORM=1; EXPLICIT_SKIP=1 ;;
    --skip-go)                SKIP_GO=1; EXPLICIT_SKIP=1 ;;
    --skip-helm)              SKIP_HELM=1; EXPLICIT_SKIP=1 ;;
...
if [[ -n "$ONLY_TOOLS" ]]; then
  (( EXPLICIT_SKIP == 0 )) || {
    err "--only cannot be combined with --skip-python/terraform/go/helm"
    exit 3
  }
  SKIP_PYTHON=1
  SKIP_TERRAFORM=1
  ...
  SKIP_HELM=1
  selected=0
  IFS=',' read -r -a only_items <<< "$ONLY_TOOLS"
  for tool in "${only_items[@]}"; do
    tool="$(printf '%s' "$tool" | tr -d '[:space:]')"
    case "$tool" in
      python)    SKIP_PYTHON=0 ;;
      terraform) SKIP_TERRAFORM=0 ;;
      go)        SKIP_GO=0 ;;
      helm)      SKIP_HELM=0 ;;
      "")        continue ;;
      *) err "unknown tool in --only: $tool (see --list-tools)"; exit 3 ;;
    esac
    selected=$((selected + 1))
  done
  (( selected > 0 )) || { err "--only needs at least one tool id"; exit 3; }
fi
...
USE_MISE=0
if [[ "$MANAGER" == "mise" ]] \
   && (( SKIP_PYTHON == 0 || SKIP_TERRAFORM == 0 || SKIP_GO == 0 )); then
  USE_MISE=1
fi
...
shell_setup_required() {
  (( USE_MISE == 1 )) && return 0
  (( SKIP_PYTHON == 0 || SKIP_GO == 0 )) && return 0
  [[ "$MANAGER" == "tenv" ]] && (( SKIP_TERRAFORM == 0 )) && return 0
  return 1
}
...
# already answers on a machine this script refuses to run on, and a dry run
# writes nothing either, so it answers there too. A real run still exits 2 at
# every one of these.
preflight_fail() {
  if (( DRY_RUN == 1 )); then
    warn "$1"
...
if ! command -v brew >/dev/null 2>&1; then
  ...
  preflight_fail "Homebrew not found. Install it first (or run ./install_apps.sh)."
fi
...
# ---------------------------------------------------------------------------
if (( SETUP_SHELL )) && shell_setup_required; then
  hr
...
export PATH="$HOME/go/bin:$PATH"'
    ...
    if (( SKIP_TERRAFORM == 0 )) && [[ "$MANAGER" == "tenv" ]]; then
...
RC
    ...
    info "no shell setup is required for the selected tools."
  fi
fi
