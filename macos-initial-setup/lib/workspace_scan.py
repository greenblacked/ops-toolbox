#!/usr/bin/env python3
"""Classify VS Code workspaceStorage entries as live, stale or unresolved.

VS Code creates ``workspaceStorage/<hash>/`` for every folder ever
opened and never garbage-collects them. This module decides which of those
...
Output is four NUL-delimited fields per entry::

    <status>\\0<entry_dir>\\0<reason>\\0<project_path>\\0

NUL rather than tabs/newlines because macOS paths may legally contain both.
"""
...
def entry_size(path: str) -> int:
    """Best-effort bytes used by an entry, without following symlinks."""
    total = 0
    pending = [path]
    while pending:
        current = pending.pop()
...
def summarize(results):
    """Return deterministic counts and byte totals for scanner results."""
    counts = {LIVE: 0, STALE: 0, UNRESOLVED: 0}
    sizes = {LIVE: 0, STALE: 0, UNRESOLVED: 0}
    for item in results:
        status = item["status"]
        counts[status] += 1
        sizes[status] += entry_size(item["dir"])
    return {
...
def main(argv=None) -> int:
    ...
    output.add_argument(
        "--summary",
        action="store_true",
        help="emit JSON counts and byte totals without listing individual entries",
    )
    ...
    for item in results:
        # NUL-delimit every field, not just every record. Tabs and newlines are
        # both valid in macOS paths; an in-band tab must never redirect a
        # cleanup decision to a different directory.
        for field in (item["status"], item["dir"], item["reason"], item["path"]):
            out.write(field.encode("utf-8", "surrogateescape") + b"\0")
    out.flush()
    return 0
