...
[![RouterOS](https://img.shields.io/badge/RouterOS-7.24.4-blue.svg)](mikrotik/README.md)
...
- MikroTik RouterOS 7.24.4 — scripts are RouterOS scripting language (`.lua`
...
- `stay_fresh.sh` handles recurring maintenance: system, application, and AI
  tool caches; old user logs; old downloads and orphaned launch agents
  (reported, removed on request); Homebrew upgrades; Docker/OrbStack cleanup;
  Xcode extras; Helm and krew plugins; `gcloud`; dev-tool caches including
  `uv` and kubectl's discovery cache; version reporting; a read-only report
...
flags, suggested scheduler entries, and RouterOS 7.24.4-specific gotchas
...
| [`mikrotik/`](mikrotik/) | **Integration** tests against a real **RouterOS 7.24.4 CHR** in QEMU, API-driven `pytest`. Slow (QEMU boot); excluded from the default selection. | [`mikrotik/tests/README.md`](mikrotik/tests/README.md) — `./mikrotik/tests/run.sh` |
