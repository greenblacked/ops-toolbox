...
# `--quiet` demonstrates the automation contract: successful chatter can be
# suppressed, while errors and dry-run plans stay visible.
#
...
info() { (( ${QUIET:-0} == 1 )) || printf "%s[info]%s %s\n" "$C_BLUE"   "$C_RESET" "$*"; }
ok()   { (( ${QUIET:-0} == 1 )) || printf "%s[ ok ]%s %s\n" "$C_GREEN"  "$C_RESET" "$*"; }
...
Usage:
  $(basename "$0") [--target NAME] [--dry-run] [--quiet]
