...
#
# Long-running model (fast: reuses one container):
#   ./run.sh up                       # start dev container in the background
#   ./run.sh kitchen verify           # exec inside the running container
#   ./run.sh cookstyle cookbooks
...
#
# One-shot model:
#   ./run.sh --once kitchen list
#   ./run.sh --once --shell
#
# Maintenance:
#   ./run.sh --rebuild                # rebuild image after Gemfile/Dockerfile change
set -euo pipefail
...
# When the entrypoint sees Ruby tooling it prefixes `bundle exec`; yamllint,
# docker, bash, sh are passed through. Same behavior whether we `exec` or
# `run --rm` because the entrypoint is set in the image.
exec_into() {
  "${COMPOSE[@]}" exec "$SERVICE" /entrypoint.sh "$@"
}
