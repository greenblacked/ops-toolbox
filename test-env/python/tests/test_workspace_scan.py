...
class CliTestCase(unittest.TestCase):
    ...
    def test_records_are_nul_delimited_with_four_fields(self):
        ...
        records = [fields[i:i + 4] for i in range(0, len(fields), 4)]
        statuses = {record[0].decode("utf-8") for record in records}
        self.assertEqual(statuses, {LIVE, STALE})

    def test_tabs_and_newlines_in_path_do_not_split_fields(self):
        # macOS permits both in filenames. Every field therefore needs NUL,
        # not just every logical record.
        weird = os.path.join(self.tmp, "two\nlines\tand-a-tab")
        os.makedirs(weird)
        entry = self.add("entry\twith-tab", weird)

        out = self.run_scanner().stdout
        fields = out.split(b"\0")[:-1]
        self.assertEqual(len(fields), 4)
        self.assertEqual(fields[1].decode("utf-8"), entry)
        self.assertEqual(fields[3].decode("utf-8"), weird)
