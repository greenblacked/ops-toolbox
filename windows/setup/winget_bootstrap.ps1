...
.DESCRIPTION
    ...
    four verbs and the same exit codes, so the two machines are managed the same
    way:
...
param(
    ...
    [ValidateSet('export', 'check', 'import', 'diff')]
    [string]$Action,
...
switch ($Action) {
    ...
    'import' {
        ...
        if ($DryRun) {
            ...
            $missing = $wanted | Where-Object { $_ -notin $installed }
            Write-Info "(dry-run) would install $($missing.Count) missing package(s) from $File"
            foreach ($id in $missing) { Write-Host "WOULD    $id" -ForegroundColor Cyan }
