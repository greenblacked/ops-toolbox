...
Usage:
  $(basename "$0") [--dry-run] [--remote origin] [--no-fetch] [--force]
...
if (( NO_FETCH == 0 )); then
  if (( DRY_RUN == 1 )); then
    ...
    # Still prune locally so the report reflects reality rather than a stale
    # snapshot. This writes only to remote-tracking refs, never to a branch.
