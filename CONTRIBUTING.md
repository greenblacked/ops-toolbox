...
The same conventions also exist as task-scoped agent skills on a local
checkout, for an automated agent that would rather load the rules for the file
it is editing than read this document end to end. That directory is gitignored
and is not on GitHub. They are derived from this file and from
