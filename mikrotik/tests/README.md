# MikroTik script tests (RouterOS 7.24.4)

[Ops Toolbox](../../README.md) / **MikroTik script tests (RouterOS 7.24.4)**

Integration tests that run **real RouterOS CHR 7.24.4** in QEMU inside Docker and
exercise every `*.lua` in the package — `../core/` and `../features/` alike.
Two services run side by side:

- `chr` — Alpine + QEMU + the official CHR 7.24.4 disk (talks to host on
...
2. **Source acceptance** — every `.lua` under `mikrotik/` is added as a
   ...
   time, so this catches syntax issues against the live 7.24.4 parser.
...
Items 4 and 5 are written and **currently xfail**, because RouterOS 7.24.4 CHR
...
python3 mikrotik/tests/routeros_version.py check --version 7.24.4
```
