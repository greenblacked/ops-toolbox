...
# check, and the log has to say which helper it looked for.
:local Send "";
...
# handed over through a global for the length of this run and cleared at the
# end. No underscore in the name, for the reason in the header.
:global StayFreshSendRef;
:if ($HaveSend) do={ :set StayFreshSendRef $Send; }
...
:do {
    ...
    :set FirmwareLine "";
    :set FirmwareBehind false;
}

:local Head ("<b>" . $DeviceName . ":</b> ");
:local Detail ("\0A\0AChannel: <code>" . $channel . "</code>" . \
               "\0AInstalled: <code>" . $installed . "</code>" . \
               "\0ALatest: <code>" . $latest . "</code>" . \
               "\0AStatus: <code>" . $status . "</code>" . \
               $FirmwareLine . \
               "\0AUptime: <code>" . $Uptime . "</code>" . \
               "\0AFree storage: <code>" . $FreeHdd . " MiB</code> / <code>" . $TotalHdd . " MiB</code>");
:local Mode "";
:if ($DryRun) do={ :set Mode "\0A\0A<i>dry run: nothing was changed</i>"; }
...
:if ([:len $why] > 0) do={
    ...
    $Notify ("\E2\9A\A0\EF\B8\8F " . $Head . "RouterOS update check FAILED, nothing installed." . \
             "\0AReason: <code>" . $why . "</code>" . $Detail . $Mode);
    :set StayFreshSendRef;
    :return "";
}
...
:if ([:typeof [:find $status "New version is available"]] != "nil") do={
    ...
    :do {
        ...
        :if ($HaveSend) do={
            :do {
                $Send MessageText=("\E2\9A\A0\EF\B8\8F " . $Head . "RouterOS install FAILED: the router did not accept " . \
                                   "<code>/system package update install</code>. Still on <code>" . $installed . "</code>.");
            } on-error={
                :log warning "stay_fresh: could not send the install-failure message";
            }
        }
    }
    :return "";
