...
Options:
  ...
  --only GROUPS     Install only a comma-separated subset of:
                    clis,python,go,terraform,helm
  --list-groups     Print the values accepted by --only and exit
...
while (( $# > 0 )); do
  case "$1" in
    ...
    --list-groups)    printf '%s\n' clis python go terraform helm; exit 0 ;;
    --skip-python)    SKIP_PYTHON=1; SKIP_USED=1 ;;
    --skip-go)        SKIP_GO=1; SKIP_USED=1 ;;
    --skip-terraform) SKIP_TERRAFORM=1; SKIP_USED=1 ;;
    --skip-helm)      SKIP_HELM=1; SKIP_USED=1 ;;
    --skip-clis)      SKIP_CLIS=1; SKIP_USED=1 ;;
    -h|--help)        usage; exit 0 ;;
    *)
...
if [[ -n "$ONLY" ]]; then
  ...
  for group in "${selected[@]}"; do
    case "$group" in
      ...
      helm)      SKIP_HELM=0 ;;
      *) err "unknown --only group: $group (see --list-groups)"; exit 3 ;;
    esac
  done
fi
...
# --- toolchains ------------------------------------------------------------
if (( SKIP_PYTHON && SKIP_GO && SKIP_TERRAFORM && SKIP_HELM && SETUP_SHELL == 0 )); then
  info "skipped: toolchains"
elif [[ "$MANAGER" == "mise" ]]; then
  step "mise"
  if have mise; then
