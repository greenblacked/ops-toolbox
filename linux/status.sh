...
if want timer; then
  if ! command -v systemctl >/dev/null 2>&1; then
    ...
    # The unit name has to match what systemd/stay_fresh_timer.sh installs,
    # which is NAME="ops-toolbox-stay-fresh" — every unit path and systemctl
    # call in that script is built from it. Asking about "stay-fresh.timer"
    # named a unit nothing ever creates, so this branch could not be reached:
    # on a machine with the timer installed, enabled and running, the section
    # told you to install the thing that was already installed.
    if systemctl --user is-enabled ops-toolbox-stay-fresh.timer >/dev/null 2>&1 \
       || systemctl --user is-active ops-toolbox-stay-fresh.timer >/dev/null 2>&1; then
      ok "timer     ops-toolbox-stay-fresh.timer is enabled or active"
    else
