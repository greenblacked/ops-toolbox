...
# pruning, container cleanup, and a report of what still needs a reboot or a
# process restart.
#
...
Usage:
  $(basename "$0") [--dry-run] [--yes] [--verbose] [--no-sudo]
                  [--only STEPS | --skip-* ...]
...
Options:
  ...
  --only STEPS       Run only a comma-separated subset of: packages,journal,
                     caches,containers,flatpak,snap
  --list-steps       Print the values accepted by --only and exit
...
while (( $# > 0 )); do
  case "$1" in
    ...
    --list-steps)      printf '%s\n' packages journal caches containers flatpak snap; exit 0 ;;
    --skip-packages)   SKIP_PACKAGES=1; SKIP_USED=1 ;;
    --skip-journal)    SKIP_JOURNAL=1; SKIP_USED=1 ;;
    --skip-caches)     SKIP_CACHES=1; SKIP_USED=1 ;;
    --skip-containers) SKIP_CONTAINERS=1; SKIP_USED=1 ;;
    --skip-flatpak)    SKIP_FLATPAK=1; SKIP_USED=1 ;;
    --skip-snap)       SKIP_SNAP=1; SKIP_USED=1 ;;
    -h|--help)         usage; exit 0 ;;
    *)
...
if [[ -n "$ONLY" ]]; then
  if (( SKIP_USED )); then
    err "--only cannot be combined with --skip-*; use one scoping style"
    exit 3
  fi
  SKIP_PACKAGES=1
  SKIP_JOURNAL=1
  ...
  for selected_step in "${selected[@]}"; do
    case "$selected_step" in
      packages)   SKIP_PACKAGES=0 ;;
      journal)    SKIP_JOURNAL=0 ;;
      caches)     SKIP_CACHES=0 ;;
      containers) SKIP_CONTAINERS=0 ;;
      flatpak)    SKIP_FLATPAK=0 ;;
      snap)       SKIP_SNAP=0 ;;
...
# Processes still running old libraries. Reboot-pending is the kernel; this
# is sshd holding a replaced libc. Same split as system_doctor.sh. A missing
# tool is silence rather than a skip: this report already has a reboot line,
# and needrestart is absent on most containers.
