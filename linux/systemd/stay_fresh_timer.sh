...
#   ./stay_fresh_timer.sh logs [--lines N]
#
# Options:
...
while (( $# > 0 )); do
  case "$1" in
    -h|--help) usage; exit 0 ;;
    install|uninstall|status|run-now|logs)
...
UNIT_EOF
    ...
    if have "$SYSTEMD_ANALYZE_CMD"; then
      lint_dir="$(mktemp -d)"
      printf '%s\n' "$service_body" >"$lint_dir/$NAME.service"
      printf '%s\n' "$timer_body"   >"$lint_dir/$NAME.timer"
      if ! "$SYSTEMD_ANALYZE_CMD" verify "$lint_dir/$NAME.service" "$lint_dir/$NAME.timer" >/dev/null 2>&1; then
        err "generated units are malformed — refusing to install"
        "$SYSTEMD_ANALYZE_CMD" verify "$lint_dir/$NAME.service" "$lint_dir/$NAME.timer" 2>&1 | sed 's/^/      /' >&2
        rm -rf "$lint_dir"
        exit 1
      fi
      rm -rf "$lint_dir"
    else
      warn "$SYSTEMD_ANALYZE_CMD not present — units are installed unverified"
    fi
    ...
    # --dry-run was parsed into TIMER_DRY_RUN but only ever consulted when
    # building ExecStart, so install --dry-run wrote both unit files and ran
    # `enable --now` — the one thing this repository promises never happens.
    # It sits ahead of require_user_manager on purpose: a preview should answer
    # on a machine that could not run the real thing, the same reasoning that
    # puts --help ahead of every preflight.
    if (( TIMER_DRY_RUN )); then
      printf "  %s(dry-run)%s would create %s\n" "$C_DIM" "$C_RESET" "$UNIT_DIR"
      printf "  %s(dry-run)%s would write %s\n"  "$C_DIM" "$C_RESET" "$SERVICE_FILE"
      printf '%s\n' "$service_body" | sed 's/^/           /'
      printf "  %s(dry-run)%s would write %s\n"  "$C_DIM" "$C_RESET" "$TIMER_FILE"
      printf '%s\n' "$timer_body" | sed 's/^/           /'
      printf "  %s(dry-run)%s would run systemctl --user daemon-reload\n" "$C_DIM" "$C_RESET"
      printf "  %s(dry-run)%s would run systemctl --user enable --now %s.timer\n" \
        "$C_DIM" "$C_RESET" "$NAME"
  ...
  logs)
    if ! have journalctl; then
      err "journalctl not found — cannot read the user journal"
      exit 2
    fi
    # Reading the journal does not need a reachable user manager, which is
    # useful precisely when `status` cannot connect after a failed login or
    # lingering setup. Preserve journalctl's real failure as exit 1.
    if ! journalctl --user -u "$NAME.service" --no-pager -n "$LINES"; then
      err "could not read logs for $NAME.service"
      exit 1
    fi
    ;;
esac
